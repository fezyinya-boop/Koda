...
def get_or_create_user(user_id, name):
    ...
    user = c.fetchone()
    if user is None:
        user = (str(user_id), name, STARTING_POINTS, 0, 0, 0, "")
    ...
    async def callback(self, interaction: discord.Interaction):
        if str(interaction.user.id) != str(self.player_id):
            return await interaction.response.send_message(
                "That's not your menu!", ephemeral=True
            )
        ...
        row = c.fetchone()
        col = "p1_cat" if row and str(interaction.user.id) == row[0] else "p2_cat"
        c.execute(f"UPDATE matches SET {col} = ? WHERE id = ?", (choice, self.match_id))
        conn.commit()
        c.execute("SELECT p1_cat, p2_cat FROM matches WHERE id = ?", (self.match_id,))
        p1c, p2c = c.fetchone()
        conn.close()

        await interaction.response.send_message(
            f"✅ **{interaction.user.display_name}** locked in **{choice}**!"
        )
        if p1c and p2c:
            await interaction.channel.send(
                f"⚔️ **MATCH AUTHORISED** — **{p1c}** vs **{p2c}** — Good luck!"
            )
...
class MatchReportingView(discord.ui.View):
    ...
    async def finalize(self, interaction, winner_id, forfeit=False):
        ...
        c.execute(
            "UPDATE matches SET winner_id=?, status='completed', notes=? WHERE id=?",
            (str(winner_id), note, self.match_id)
        )
        conn.commit()
        conn.close()
        ...
        if forfeit:
            embed.description += "\n*(Opponent didn't respond within the forfeit window)*"
        embed.add_field(
            name="RESULTS",
            value=(f"📈 **{w_mem.display_name}**: `+{pts} {RATING_LABEL}`\n"
                   f"📉 **{l_mem.display_name}**: `-{pts} {RATING_LABEL}`"),
            inline=False
        )
        embed.set_footer(text=f"{SERVER_NAME} • Elo updated")
    ...
    async def report_p1(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in [self.p1.id, self.p2.id]:
            return await interaction.response.send_message(
                "You're not a participant in this match.", ephemeral=True
            )
        ...
        if p1_rep and p2_rep:
            ...
            if p1_rep != p2_rep:
                embed = discord.Embed(
                    ...
                    description=(f"**{self.p1.display_name}** and **{self.p2.display_name}** "
                                 f"reported different results.\n"
                                 f"<@&{MOD_ROLE_ID}> must resolve this via `/settle`."),
                    color=0xe74c3c
                )
                await interaction.response.edit_message(
                    content=f"<@&{MOD_ROLE_ID}>", embed=embed, view=None
                )
            else:
            ...
            if not self.forfeit_task:
                ...
                await interaction.response.edit_message(
                    content=(f"⏳ **{interaction.user.display_name}** reported. "
                             f"<@{other.id}> has **{mins} minutes** to confirm or receive an auto-loss.")
                )
            else:
                await interaction.response.edit_message(
                    content="⏳ Still waiting for opponent to confirm…"
                )
...
class ChallengeView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if USE_CATEGORIES:
            view.add_item(CategorySelect(match_id, self.p1.id, self.p1.display_name))
            view.add_item(CategorySelect(match_id, self.p2.id, self.p2.display_name))
...
async def refresh_leaderboard(guild):
    try:
        ...
        if msg and (not saved_msg_id or msg.id != saved_msg_id):
            ...
            c = conn.cursor()
            c.execute(
                "INSERT OR REPLACE INTO config (key, value) VALUES ('leaderboard_msg_id', ?)",
                (str(msg.id),)
            )
            conn.commit()
            conn.close()
...
async def rank(interaction: discord.Interaction, user: discord.Member = None):
    ...
    embed.add_field(name="🔥 Streak",           value=f"{data[5]} win(s)",                      inline=True)
    embed.add_field(name="⚔️ Record",           value=f"{data[3]}W - {data[4]}L ({win_rate}%)", inline=True)
...
async def profile(interaction: discord.Interaction, user: discord.Member = None):
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT title, signature_move, embed_color FROM profiles WHERE user_id = ?",
              (str(member.id),))
...
async def setprofile(interaction: discord.Interaction, field: str, value: str):
    ...
    c.execute(f"UPDATE profiles SET {col_map[field]} = ? WHERE user_id = ?",
...
async def settle(interaction: discord.Interaction, winner: discord.Member, loser: discord.Member):
    ...
    if row:
        ...
        c.execute(
            "INSERT INTO matches (p1_id, p2_id, winner_id, status, notes) VALUES (?,?,?,'completed','Mod-settled')",
            (str(winner.id), str(loser.id), str(winner.id))
        )
    conn.commit()
    conn.close()
...
async def adjust(interaction: discord.Interaction, user: discord.Member, amount: int):
    data    = get_or_create_user(user.id, user.display_name)
    new_pts = max(0, data[2] + amount)
    conn    = sqlite3.connect(DB_NAME)
    c       = conn.cursor()
    c.execute("UPDATE users SET points=? WHERE user_id=?", (new_pts, str(user.id)))
    conn.commit()
    conn.close()
...
async def add_category(interaction: discord.Interaction, name: str, tier: str = ""):
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO categories (name, tier) VALUES (?, ?)", (name, tier))
        conn.commit()
        await interaction.response.send_message(
...
async def fix_database(interaction: discord.Interaction):
    ...
    try:
        ...
        for col, default in [("streak", "INTEGER DEFAULT 0"), ("history", "TEXT DEFAULT ''")]:
            try:
                c.execute(f"ALTER TABLE users ADD COLUMN {col} {default}")
            except Exception:
                pass
        conn.commit()
        await interaction.response.send_message("✅ Database patched successfully.", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(f"❌ Patch error: `{e}`", ephemeral=True)
    finally:
        conn.close()
