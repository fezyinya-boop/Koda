...
class CategorySelect(discord.ui.Select):
    def __init__(self, match_id: int, player_id: int, player_name: str, slot: str):
        """
...
class MatchReportingView(discord.ui.View):
    ...
    async def finalize(self, interaction: discord.Interaction | None, winner_id: int, forfeit: bool = False):
        ...
        w_data = await get_or_create_user(w_mem.id, w_mem.display_name)
        l_data = await get_or_create_user(l_mem.id, l_mem.display_name)
        ...
        await update_user_stats(w_mem.id, r1 + pts, w_data[3] + 1, w_data[4], w_data[5] + 1, w_hist)
        await update_user_stats(l_mem.id, r2 - pts, l_data[3], l_data[4] + 1, 0, l_hist)
...
class ChallengeView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if USE_CATEGORIES:
            view.add_item(CategorySelect(match_id, self.p1.id, self.p1.display_name, slot='p1'))
            view.add_item(CategorySelect(match_id, self.p2.id, self.p2.display_name, slot='p2'))
...
async def rank(interaction: discord.Interaction, user: discord.Member = None):
    ...
    data   = await get_or_create_user(member.id, member.display_name)
    pts    = data[2]
...
async def settle(interaction: discord.Interaction, winner: discord.Member, loser: discord.Member):
    ...
    w_data = await get_or_create_user(winner.id, winner.display_name)
    l_data = await get_or_create_user(loser.id,  loser.display_name)
    r1, r2 = w_data[2], l_data[2]
    pts    = elo_gain(r1, r2)
    ...
    await update_user_stats(winner.id, r1 + pts, w_data[3] + 1, w_data[4],     w_data[5] + 1, w_hist)
    await update_user_stats(loser.id,  r2 - pts, l_data[3],     l_data[4] + 1, 0,             l_hist)
...
async def adjust(interaction: discord.Interaction, user: discord.Member, amount: int):
    data    = await get_or_create_user(user.id, user.display_name)
...
async def tourney_start(interaction: discord.Interaction):
    ...
    player_data = [(p, await get_or_create_user(p.id, p.display_name)[2]) for p in tournament_players]
